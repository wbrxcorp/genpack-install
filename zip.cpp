    ...
    const std::filesystem::path& system_image, const ZipOptions& options)
{
    ...
    if (image.is_regular_file(extlinux_conf_image_path)) {
        std::cout << "Collecting boot files for U-Boot extlinux..." << std::endl;
        auto add_from_image = [&image,&entries](const std::string& path, const std::string& name) {
            if (!image.is_regular_file(path)) {
                throw std::runtime_error(name + " referenced by extlinux.conf does not exist in system image");
            }
            //else
            put(entries, ZipEntry {
                name, ZipEntry::Source::ImageFile, {}, path, "",
                image.file_mtime(path).value_or(std::time(nullptr)),
                image.file_size(path).value_or(0)
            }, "a U-Boot extlinux boot file");
        };
        for (const auto& relative:get_files_referenced_by_extlinux_conf(image)) {
            add_from_image(("boot" / relative).generic_string(), relative.generic_string());
        }
        add_from_image(extlinux_conf_image_path, extlinux_conf_dest_path);
        std::cout << "Done." << std::endl;
    }
